...
use crate::client::gui::bb_theme::color::{CONTAINER_COLOR, DARKER_CONTAINER_COLOR, HIGHLIGHTED_CONTAINER_COLOR};
...
pub fn new_workout_widget_button<'a>() ->Button<'a, Message, Theme, Renderer> {
    button(column!(Space::with_height(Length::Fill), format_button_text(text("New workout")).size(DEFAULT_TITLE_FONT_SIZE),
                   format_button_text(text("+").size(65)),
                    Space::with_height(Length::Fill)
           ).width(BUTTONS_WIDTH).height(NEW_WORKOUT_BUTTON_HEIGHT).align_x(Horizontal::Center)
    ).style(|_,status|{
        create_button_style(status, BORDER, HIGHLIGHTED_CONTAINER_COLOR, DARKER_CONTAINER_COLOR, CONTAINER_COLOR)
    })
        .width(BUTTONS_WIDTH)
...
pub fn new_preset_widget_button<'a>() -> Button<'a, Message, Theme, Renderer> {
    button(column!(Space::with_height(Length::Fill),format_button_text(text("New preset")).size(DEFAULT_TITLE_FONT_SIZE),
                   format_button_text(text("+").size(65)),
                Space::with_height(Length::Fill)
           ).width(BUTTONS_WIDTH).height(NEW_PRESET_BUTTON_HEIGHT).align_x(Horizontal::Center)
    ).style(|_,status|{
        create_button_style(status, BORDER, HIGHLIGHTED_CONTAINER_COLOR, DARKER_CONTAINER_COLOR, CONTAINER_COLOR)
    })
        .width(BUTTONS_WIDTH)
