        ...
        + text::Renderer<Font = iced::Font>
{
    pub fn default_workout_preset_widget() -> Self {
        WorkoutWidget {
            ...
            image: Some(Image::new(image::Handle::from_path("assets/images/default_preset.png"))),
    ...
    where Renderer: image::Renderer + text::Renderer
{
    ...
    fn draw(&self,
        ...
        if self.image.is_none() {
            ...
            description_start_location_y = layout.bounds().y + 2.0 * INDENT + 1.5 * self.title_font_size;
        } else {
            title_start_location_y = layout.bounds().y + 3.0 * INDENT + IMAGE_HEIGHT;
            description_start_location_y = layout.bounds().y + 3.0 * INDENT + IMAGE_HEIGHT
