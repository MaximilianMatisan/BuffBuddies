...
pub fn chop_dates(graph_widget_state: &GraphWidgetState, x_values: Vec<NaiveDate>) -> Vec<NaiveDate> {
    let chop_start_index: isize =
...
pub fn calculate_points(graph_widget_state: &GraphWidgetState, y_values: Vec<Kg>) -> Vec<Point> {
    ...
    let block_width =
        (CHART_WIDGET_WIDTH - crate::client::gui::bb_widget::chart_widget::graph::GRAPH_PADDING * 2.0) / graph_widget_state.points_to_draw as f32; //division with 0 is not possible since limit is 1
    //PADDING UP AND DOWN:  UP FOR Y-AXIS-ARROW SPACE,DOWN FOR X-LABELS
    let block_height =
        (CHART_WIDGET_HEIGHT - crate::client::gui::bb_widget::chart_widget::graph::GRAPH_PADDING * 2.0) / crate::client::gui::bb_widget::chart_widget::graph::FREQUENCY_OF_Y_AXIS_LABELS as f32;
    let mut current_x = CHART_WIDGET_WIDTH - crate::client::gui::bb_widget::chart_widget::graph::GRAPH_PADDING;
    ...
    let height_graph: f32 =
        CHART_WIDGET_HEIGHT - crate::client::gui::bb_widget::chart_widget::graph::GRAPH_PADDING * 2.0 - height_padding_for_arrow - x_axis_padding;
    let lowest_point_graph: f32 = CHART_WIDGET_HEIGHT - crate::client::gui::bb_widget::chart_widget::graph::GRAPH_PADDING - x_axis_padding;
