...
use crate::client::gui::bb_theme::scrollable::{create_scrollable, ScrollableExtension, ScrollableStyle, TAB_SCROLLABLE_PADDING, TAB_SCROLLABLE_WIDTH, WIDGET_SCROLLABLE_WIDTH};
...
impl App {
    pub fn homescreen(&self) -> Element<Message> {
        let welcome_back_text = format_button_text(text("Welcome back!")).size(WELCOME_BACK_TEXT_SIZE);
        ...
        let activity_widget_with_mascot:Element<Message> = match self.widget_manager.activity_widget.current_scope {
            DateScope::Year => {
                ...
                ).align_y(Vertical::Center).height(HEIGHT_MASCOT).into()
            }
            _ =>  Row::new()
                .push(activity_widget_with_welcome)
                .push(Space::with_width(Length::Fixed(DISTANCE_MASCOT_ACTIVITY_WIDGET)))
                .push(mascot_with_message)
                .align_y(Vertical::Center).height(HEIGHT_MASCOT).into()
        };

        let track_new_workout_text = text("Track a new workout!").font(FIRA_SANS_EXTRABOLD).color(color::TEXT_COLOR).size(SECONDARY_TEXT_SIZE);
        ...
        let home_screen_content = Column::new()
            ...
            .push(Space::with_height(Length::Fixed(SPACING)))
            .push(
                create_scrollable(workout_presets,self.mascot_manager.selected_mascot,ScrollableStyle::Mascot)
                    .add_horizontal_scrollbar(WIDGET_SCROLLABLE_WIDTH,0.0)
            )
        ...
        ).add_vertical_scrollbar(TAB_SCROLLABLE_WIDTH, TAB_SCROLLABLE_PADDING)
            .into()
    }
}