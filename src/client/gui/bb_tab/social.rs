...
use iced::widget::{container, text, Column, Row, Scrollable, Space};
...
impl UserInterface {
    pub fn social_screen(&self) -> Element<Message> {
        ...
        for friend in &friends {
            friend_buttons = friend_buttons
                .push(friend_button(&self.app, friend))
        }
        let scrollable_friends = Scrollable::new(friend_buttons)
            .direction(Direction::Horizontal(Scrollbar::new()));

        let friends_with_title = Column::new()
            .push(text("Buddies").font(FIRA_SANS_EXTRABOLD).color(TEXT_COLOR).size(40))
        ...
        for user in &non_friend_users {
            user_buttons = user_buttons
                .push(user_profile_button(&self.app.mascot_manager.selected_mascot, user))
        }

        let user_navigation = Row::new() //TODO add text_input element for search
            .push(text("Find other users").font(FIRA_SANS_EXTRABOLD).color(TEXT_COLOR).size(24));

        let user_elements = Column::new()
            .push(user_navigation)
            .push(user_buttons);
